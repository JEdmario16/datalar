...
class PropertySchema(BaseModel):
    ...
    for_rent: bool = Field(
        ..., description="Indica se o imóvel está disponível para aluguel"
    )
    for_sale: bool = Field(
        ..., description="Indica se o imóvel está disponível para venda"
    )
    iptu: float = Field(..., description="Valor do IPTU do imóvel")
    sale_price: float | None = Field(
        None, description="Preço de venda do imóvel, se aplicável"
    )
    rent_price: float | None = Field(
        None, description="Preço de aluguel do imóvel, se aplicável"
    )
    ...
    property_type: PropertyType = Field(
        ..., description="Tipo de imóvel (residencial, comercial, etc.)"
    )
    ...
    source: str = Field(
        ..., description="Fonte de onde as informações do imóvel foram extraídas"
    )
    scraped_at: str = Field(
        ..., description="Data e hora em que as informações foram extraídas"
    )
    source_id: str = Field(
        ..., description="Identificador único da fonte de dados do imóvel"
    )
