...
from datalar.scrapers.zap_imoveis.sdk.routes.base import NotFoundError, Route
...
def test_raise_for_status_method_should_not_be_called_when_404_and_raise_for_status_false():
    ...
    with patch.object(
        route, "raise_for_status", wraps=route.raise_for_status
    ) as mock_method:
...
def test_route_when_log_error_is_true():
    api = ZapGlueAPI(
        SDKConfig(LOG_LEVEL="DEBUG", LOG_REQUESTS=True, LOG_RESPONSES=True)
    )
    ...
    with patch.object(api.logger, "error") as mock_log_error:
        ...
        mock_log_error.assert_called_once_with(
            f"Error in {route.__class__.__name__}: Resource not found: {api.BASE_URL}test/listings"
        )
...
def test_route_when_log_response_is_true():
    api = ZapGlueAPI(
        SDKConfig(LOG_LEVEL="DEBUG", LOG_REQUESTS=False, LOG_RESPONSES=True)
    )
    ...
    resp = responses.Response(
        ...
        url=api.BASE_URL + "test/listings",
        json={"data": "test"},
        status=200,
    )
    responses.add(resp)
    with patch.object(api.logger, "debug") as mock_log_response:
        ...
        mock_log_response.assert_called_once_with(
            f"Response from {resp.url} (status: {response.status_code}): {response.content.decode('utf-8', errors='ignore')}"
        )
